...
ALTER TABLE emp01 MODIFY (job number(20));
...
-- ename컬럼 데이터의 최고 사이즈 확인
--
select ename
from emp
where lengthb(ename) in (select max(lengthb(ename)) from emp);
...
-- 데이터 사이즈보다 작게 수정
-- 불가
ALTER TABLE emp01 MODIFY(ename varchar2(5));
...
ALTER TABLE COMPANY add(career number(10));


CREATE TABLE jobapplicant as select * from company where 1=0;

select * from jobapplicant;

ALTER TABLE jobapplicant RENAME COLUMN cname TO jname;